...
// class: Lab_3_Dzyhovskyi_V_I
// Copyright (c) 2021 Dzyhovskyi V. I.
//==============================================================================
package Lab_3_Dzyhovskyi_V_I;
 ...
 */
public class Lab_3_Dzyhovskyi_V_I extends Application {
    
    Pane root_pane = new Pane();
    Group group = new Group();
    Rectangle[] rect = new Rectangle[4];
    Text[] rectText = new Text[4];
    RadioButton[] onRB = new RadioButton[4];
    RadioButton[] offRB = new RadioButton[4];
    ToggleGroup[] tg = new ToggleGroup[4];
    Text sumText;
    ...
    private void createRadioButtons(int i, double dx) {
            tg[i] = new ToggleGroup();
            onRB[i] = new RadioButton("on");
            onRB[i].setLayoutX(dx);
            onRB[i].setLayoutY(180);
            onRB[i].setToggleGroup(tg[i]);
            onRB[i].setSelected(true);
            onRB[i].setOnAction((ActionEvent event) -> {
                th[i].resume(); 
            });
            offRB[i] = new RadioButton("off");
            offRB[i].setLayoutX(dx);
            offRB[i].setLayoutY(210);
            offRB[i].setToggleGroup(tg[i]);
            offRB[i].setOnAction((ActionEvent event) -> {
                th[i].suspend(); 
            });
    }
    ...
    private void createGraphNodes() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            createRadioButtons(i, dx);
            dx += 100;
            group.getChildren().addAll(rect[i], rectText[i], onRB[i], offRB[i]);
        }
        sumText = new Text(60, MAX_HEIGHT + 130, "0");       
    ...
    public void start(Stage stage) {
        stage.setTitle("Лабораторная работа №1. Дзыговского В.И.");
        stage.setResizable(false);
        createGraphNodes();
        createThreads();
        Scene scene = new Scene (root_pane, 400, MAX_HEIGHT + 150, Color.TRANSPARENT);
